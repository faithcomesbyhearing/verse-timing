...
def load_excel_file(file):
    ...
    #Load the file
    if str(fe).upper()=='.XLSX':
        input_df=(pd.read_excel(file, sheet_name=core_script_sheet_name, encoding='utf-8',index_col=None)).astype(str)

    elif fe=='.csv':
        input_df = (pd.read_csv(file, sheet_name=core_script_sheet_name, encoding='utf-8',index_col=None)).astype(str)
    ...
    try:
        ...
        for index in range1(line_start_index+1,line_end_index):
            ...
            do_not_combine_boolean_flag = (script_df[input_column_name][index]).__contains__(
                combine_tag.upper())
            # IMP: assumes the timing cues are inside the || string #Detect Minute as well, used || for detection to be safe for foreign languages
            if (script_df[input_column_name][index]).__contains__('|') and (
            ''.join(filter(str.isdigit, re.split('\|+', script_df[input_column_name][index])[1]))).isdigit():
                # postSilence = ''.join(filter(str.isdigit, re.split('\|+', script_df[input_column_name][line_start_index])[1]))
            ...
            for i,check_word in enumerate(line_content):
                ...
                if re.search(verse_find_string, check_word) is not None:

                    if (re.search(verse_find_string, check_word).group(1)).__contains__(verse_end_string) or int((re.search(verse_find_string,check_word).group(1)).split(verses_split_string)[0])<=119:

                        if (re.search(verse_find_string, check_word).group(1)).__contains__(verse_end_string):
                            content=re.search(verse_find_string, check_word).group(1)

                            verse_num_list.append(content.split(verse_start_string)[-1])
                            verse_ind_list.append(i)
                            current_verse_num=content.split(verse_start_string)[-1]
                            if chapter == 1 and current_verse_num==40: print('1',current_verse_num)

                        else:
                            verse_num_list.append((re.search(verse_find_string, check_word).group(1)))
                            verse_ind_list.append(i)
                            current_verse_num=(re.search(verse_find_string, check_word).group(1))
