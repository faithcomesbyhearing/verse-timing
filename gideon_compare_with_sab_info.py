...
required_args.add_argument(
    '-sab_timings_find_string', required=True, nargs=1, type=str, help='EX: MRK to get MRK1,ESV_MRK_01 etc.')
...
optional_args.add_argument('-check_secs', nargs=1, type=str,default=['1'],help='Mark if above check secs')
...
else:strict_comparison=1
print_chapter=args.print_chapter[0]
sab_find_string=args.sab_timings_find_string[0]
...
for each_chapter in chapter_list:
    ...
    dummy_target_list=df[target][(df['verse_number'] != str(0)) & (df['chapter'] == str(each_chapter))]
    uniq_target=dummy_target_list.unique()
    if each_chapter==str(1): print('target_list', target_list,'uniq',uniq_target)

    if float(each_chapter)<10:
        #Get cue info file
        cue_info_chapter_file=glob.glob(input_dir+'/*'+sab_find_string+'-'+'0'+str(each_chapter)+'*timing.txt')[0]
        print(cue_info_chapter_file)
        ...
        aeneas_chapter_file = glob.glob(input_dir + '/*'  + book_find_string + '_'+'0' + str(each_chapter)+'*' + aeneas_adjusted_file)[0]
    else:
        #Get cue info file
        cue_info_chapter_file=glob.glob(input_dir+'/*'+sab_find_string+'-'+str(each_chapter)+'*timing.txt')[0]
        print(cue_info_chapter_file)
    ...
    cue_target=open(cue_info_chapter_file,'r',encoding='utf-8').readlines()
    ...
    with open(audition_file,'w') as aud:
        ...
        for each_target in uniq_target:
            ...
            if ind<=len(open(cue_info_chapter_file).readlines(  )):
                ...
                if each_chapter==str(1):print('indices',indices)
                #print(each_target)
                #print(indices)
                aeneas_duration = 0
                ...
                aud_duration=0
                aud_text=''
                # print(((aeneas_target[0]).strip('\n')).split('\t'))
                # print(((aeneas_target[1]).strip('\n')).split('\t'))
                # print(((aeneas_target[58]).strip('\n')).split('\t'))

                for each_index in indices:
                    #print(each_index)
                    aeneas_times = ((aeneas_target[each_index]).strip('\n')).split('\t')
                    #print('aen_times',aeneas_times)
                    aeneas_duration = float(aeneas_times[1]) - float(aeneas_times[0])
                    #print('aen_dur',aeneas_duration)
                ...
                if strict_comparison==1:
                    difference=(round(abs(cue_start - aeneas_start), 1))
                    #print(each_chapter)
                    if each_chapter==str(1): print(each_target, cue_start, aeneas_start,difference)
                elif strict_comparison==2:difference=(round(abs(cue_start - aeneas_start), 1))+(round(abs(cue_duration - aeneas_duration), 1))
                # print(difference)
                difference_list.append(difference)
                if difference>=1:print('aeneas_chapter_file',aeneas_chapter_file,'chapter',each_chapter,'each_target',each_target,'diff',difference,'aen',aeneas_times,aeneas_duration,'time',cue_times,cue_duration)
                # if each_chapter==print_chapter: print(cue_start,aeneas_start)

                if difference > float(args.check_secs[0]):
                    marker_name = 'A1_Check Marker '
                else:
                    marker_name = 'A1_Marker '
                ...
                if ind==1:
                    ...
                    if int(       str(each_target ).split('-')[0]      )<10:marker_number='0'+str(each_target)
    ...
    # target1_list = df[target][(df['verse_number'] != str(0)) & (df['chapter'] == str(each_chapter))]
    # uniq1_target = target1_list.unique()

    target1_list=df[target1][(df['chapter'] == str(each_chapter))]

    dummy_target1_list=df[target1][(df['verse_number'] != str(0)) & (df['chapter'] == str(each_chapter))]
    uniq1_target=dummy_target1_list.unique()
    ...
    with open(audition_file,'w') as aud:
        ...
        for each_target in uniq1_target:
            ...
            if ind==1:
                ...
                if int(      str(each_target).split('-')[0]              ) < 10:
        ...
        if each_chapter==20 : print('chapter',each_chapter,'diff',difference_list)
    ...
    #target1_list = df[target1][df['chapter'] == str(each_chapter)]
    # target1_list = df[target][(df['verse_number'] != str(0)) & (df['chapter'] == str(each_chapter))]
    # #print('target1_list',type(target1_list))
    # uniq1_target = target1_list.unique()
    ...
    #print('uniq1_target',type(uniq1_target))

    target2='verse_number'
    #target2_list = df[target2][df['chapter'] == str(each_chapter)]
    # target2_list = df[target][(df['verse_number'] != str(0)) & (df['chapter'] == str(each_chapter))]
    # uniq2_target = target2_list.unique()


    target2_list=df[target2][(df['chapter'] == str(each_chapter))]

    dummy_target2_list=df[target2][(df['verse_number'] != str(0)) & (df['chapter'] == str(each_chapter))]
    uniq2_target=dummy_target2_list.unique()
    ...
    with open(audition_file,'w') as aud:
        ...
        for each_target in uniq1_target:
            ...
            for k,kval in enumerate(target1_list):
                if int(  str(kval).split('-')[0]             )==(int(  str(each_target).split('-')[0]          )+1):
                    next_verse_index=k
                    break
            ...
            for g,each_index in enumerate(indices):
                ...
                if counter==0:
                    ...
                    if ind>1 and (float(     str(target2_list.iloc[each_index]).split('-')[0]        )-float(       str(target2_list.iloc[each_index-1]).split('-')[0]              ))==1 :

                        # if each_chapter==print_chapter:
                        #     print(each_target,target2_list.iloc[each_index])

                        #print(len(uniq2_target))
                        #Get adjustboundary1
                        for i,each_line in enumerate(uniq2_target):
                            if each_line==target2_list.iloc[each_index]:
                                ...
                                #print('i',i,'each_chapter',each_chapter)

                                adjust_boundary1=float(    ((line_cue_target[i-1]).split('\t')[0]    )     )
                                # if each_chapter==print_chapter:
                        ...
                        with open(silence_file,'r') as s:
                            for silence_bounds in s:
                                ...
                                if ind<len(aeneas_target):
                                    ...
                                    if silence_end - silence_start >= 0.4 and silence_start <= adjust_boundary1 <= silence_end and aeneas_start_previous<adjust_boundary1 < aeneas_start_next :
                                        #if each_chapter == print_chapter:
                                        ...
                                        #print('previous_verse_index',delta,'chapter',each_chapter)
                                        # aud_duration=aud_duration-delta
                                        break
                                else:
                                    if silence_end - silence_start >= 0.4 and silence_start <= adjust_boundary1 <= silence_end:
                                        #if each_chapter == print_chapter:
                                            #print("chapter-{0},verse_num-{1},verse index-{2},line_num-{3}".format(
                                                #each_chapter, each_target, each_index,
                                                #float(target2_list.iloc[each_index])))
