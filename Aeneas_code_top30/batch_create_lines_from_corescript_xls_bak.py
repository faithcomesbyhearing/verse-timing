...
def get_chapter_title(path,fileset):
    t = eyed3.load(path)
    text=t.tag.lyrics[0].text
    chapter_title=((str(text)).split('\r')[0])[1:]
    map_dict={'CHNUNV':'zh-tw','CHNUN1':'zh-cn'}
...
chapter_audio=glob.glob(input_audio_dir+'/*'+'01*01'+'*.mp3')[0]
path=chapter_audio
r = sr.Recognizer()
...
threshold_length=len(whole_text)
...
def get_large_audio_transcription(path,fileset):
    ...
    if os.path.isfile(fileset+'tmp.wav') :os.remove(fileset+'tmp.wav')
    subprocess.run(["rm", "-rf", folder_name])
    if len(whole_text)>threshold_length+10:
        whole_text=get_chapter_title(path, fileset)
    return whole_text
