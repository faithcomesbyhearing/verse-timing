...
def get_chinese_chapter_title_from_lyrics(path,fileset):
    t = eyed3.load(path)
    text=t.tag.lyrics[0].text
    chapter_title=((str(text)).split('\r')[0])[1:]
    map_dict={'CHNUNV':'zh-tw','CHNUN1':'zh-cn','FRNTLS':'fr-ch'}
    english_chapter_title="".join(mit.flatten(mit.split_at(chapter_title, pred=lambda x: x in set(remove_characters))))
    chinese_chapter_title=GoogleTranslator(source='auto', target=map_dict[(str(fileset))[:-4]]).translate(english_chapter_title)
    return chinese_chapter_title

def convert_eng_to_chinese(text):
    map_dict={'CHNUNV':'zh-tw','CHNUN1':'zh-cn','FRNTLS':'fr-ch'}
    english_chapter_title="".join(mit.flatten(mit.split_at(text, pred=lambda x: x in set(remove_characters))))
    chinese_chapter_title=GoogleTranslator(source='auto', target=map_dict[(str(fileset))[:-4]]).translate(english_chapter_title)
    return chinese_chapter_title

def get_threshold(path):
    #Get threshold length of chapoter title from matherw chapter 1 to compare against others
    #Get first chapter of the book
    mp3=path.split('/')[-1]
    book_id=list(filter(lambda x: len(x) > 0, mp3.split("_")))[0]
    chapter_audio=glob.glob(input_audio_dir+'/*'+book_id+'*01'+'*.mp3')[0]
    print(chapter_audio)
    path=chapter_audio
    r = sr.Recognizer()
    sound = AudioSegment.from_mp3(path)
    sound = sound[:30000]
    sound.export(fileset+"tmp.wav", format="wav")
    sound = AudioSegment.from_wav(fileset+'tmp.wav')
    ...
    folder_name = fileset+"audio-chunks"
    ...
    for i, audio_chunk in enumerate(chunks, start=1):
        ...
        if i==1:
            ...
            with sr.AudioFile(chunk_filename) as source:
                ...
                try:
                    text = r.recognize_google(audio_listened,language="fr-ch")
        ...
        # return the text for all chunks detected
            ...
            with sr.AudioFile(chunk_filename) as source:
                ...
                try:
                    ...
                    if chapter == '01':
                        first_whole_text += ' ' +text
                        if first_whole_text.split(' ')[-1]==first_whole_text.split(' ')[-2]:
                            first_whole_text=' '.join(first_whole_text.split(' ')[:-1])
                    if str(text).isalnum():
                        if str(text).split(' ')[0]==' ':
                            first_whole_text += text
                        else: first_whole_text += ' '+text
        # return the text for all chunks detected

        if i==target:
            break
    if os.path.isfile(fileset+'tmp.wav') :os.remove(fileset+'tmp.wav')
    subprocess.run(["rm", "-rf", folder_name])
    # number=int(''.join(filter( lambda x: x in '0123456789', whole_text )))
    t = eyed3.load(path)



    if chapter != '01' :first_whole_text = (first_whole_text.split(' ')[:-1])[0]
    elif len(first_whole_text.split(' '))>2 and first_whole_text.split(' ')[-1] == first_whole_text.split(' ')[-2]:
                                whole_text = ' '.join(first_whole_text.split(' ')[:-1])

    if first_whole_text != '' and len(''.join(filter(lambda x: x in '0123456789', first_whole_text))) > 0:
        number = int(''.join(filter(lambda x: x in '0123456789', first_whole_text)))
        literal_number = n2w(number)
        first_whole_text=first_whole_text.replace(str(number), convert_eng_to_chinese(literal_number))

    print(first_whole_text)
    threshold_length=len(first_whole_text)
    return threshold_length

# a function that splits the audio file into chunks
# and applies speech recognition
def get_large_audio_transcription(path,fileset):
    """
        ...
        # return the text for all chunks detected
            ...
            with sr.AudioFile(chunk_filename) as source:
                ...
                try:
                    ...
                    if chapter == '01':
                        whole_text += ' ' +text
                        if whole_text.split(' ')[-1] == whole_text.split(' ')[-2]:
                            whole_text = ' '.join(whole_text.split(' ')[:-1])
                    if str(text).isalnum():
                        if str(text).split(' ')[0]==' ':
                            whole_text += text
    ...
    subprocess.run(["rm", "-rf", folder_name])
    print(whole_text)
    if len(whole_text)>get_threshold(path)+10:
        t = eyed3.load(path)
        if len(t.tag.lyrics) > 0:
            whole_text = get_chinese_chapter_title_from_lyrics(path, fileset)
            if whole_text != '' and len(''.join(filter(lambda x: x in '0123456789', whole_text))) > 0:
                number = int(''.join(filter(lambda x: x in '0123456789', whole_text)))
                literal_number = n2w(number)
                whole_text=whole_text.replace(str(number), convert_eng_to_chinese(literal_number))

    #number = int(''.join(filter(lambda x: x in '0123456789', whole_text)))


    print(chapter,''.join(filter(lambda x: x in '0123456789', whole_text)))

    if chapter != '01' :whole_text = ' '.join(whole_text.split(' ')[:-1])
    elif len(whole_text.split(' '))>2 and whole_text.split(' ')[-1] == whole_text.split(' ')[-2]:
                            whole_text = ' '.join(whole_text.split(' ')[:-1])
    if whole_text != '' and len(''.join(filter(lambda x: x in '0123456789', whole_text))) > 0:
        number = int(''.join(filter(lambda x: x in '0123456789', whole_text)))
        literal_number = n2w(number)
...
def get_chapter_verses(language,input_book_chapter,output_book_chapter,target_language_column_index,script_df,original_core_script_flag):
    ...
    #
    #     word=str(get_large_audio_transcription(chapter_audio,fileset)).replace('st.', 'saint')
    #     word = word.strip()
...
if core_script_df.columns[0]=='book':
    ...
    if args.book_chapter is not None:
        ...
        map_book_df = pd.read_csv(book_to_audio_map)
        book_list=map_book_df['sabid']

        for each_book in book_list:
            chapter_list=list(core_script_df[core_script_df['book']==each_book]['chapter'].unique())
            print(each_book,chapter_list)
            #chapter_list=[int(x) for x in chapter_list]
            #print(chapter_list)
            #chapter_list=chapter_list.sort()
            #print(0,chapter_list)
            #chapter_list = [str(x) for x in chapter_list]
            #print(1,chapter_list)
            book_chapters_list.append([each_book+' '+x for x in chapter_list])
        book_chapters_list=[ item for elem in book_chapters_list for item in elem]
        print(2,book_chapters_list)

    target_language_column_index=12
    language = detect(core_script_df['verse_content1'][1])
