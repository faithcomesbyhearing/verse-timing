...
i=0
core_df = pd.read_csv(scripts_dir + '/'+input_file, encoding='utf-8',index_col=None).astype(str)
filesetid=core_df.loc[0,'filesetid']
bibleid=core_df.loc[0,'bibleid']
for each_file in glob.glob(scripts_dir+'/Batch*'):
    input_file=each_file.split('/')[-1]
    i+=1
    print('running last')
    os.system('nohup python3 test_batch_scripts_aws_multiprocess_N2.py -input_file '+input_file+' > ./mylog'+str(i)+'.log  2>&1 &')
